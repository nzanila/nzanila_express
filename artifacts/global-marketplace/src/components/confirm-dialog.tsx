...
// NoticeHost renders outside LocaleProvider (see main.tsx), so this file uses the
// hook-free tg() rather than useLocale().
  ...
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 sm:items-center sm:p-4"
    ...
    <div role="dialog" aria-modal="true" aria-label={spec.title}
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button onClick={onClose} disabled={busy} aria-label={tg('ui.close')}
...
/**
 * Root-mounted dialog host, for call sites too far from a component that owns dialog
 * state to thread it through — the storefront builder's save and upload paths. Components
 * that already hold their own `ConfirmSpec` state keep using <ConfirmDialog> directly.
