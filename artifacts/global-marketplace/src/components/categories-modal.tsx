...
export function CategoriesModal({ isOpen, onClose, initialCategory }: { isOpen: boolean; onClose: () => void; initialCategory?: string }) {
  ...
  return <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent className="w-[96vw] max-w-6xl h-[min(720px,88dvh)] p-0 overflow-hidden">
      <DialogTitle className="sr-only">Browse categories</DialogTitle>
      <div className="grid h-full min-h-0 grid-cols-[130px_minmax(0,1fr)] sm:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="overflow-y-auto overscroll-contain bg-gray-50 py-4">
          {isLoading && <p className="p-4 text-sm">Loading categories…</p>}
          {isError && <button onClick={() => refetch()} className="p-4 text-sm">Couldn’t load categories. Retry</button>}
        ...
        <section className="min-w-0 overflow-y-auto overscroll-contain p-4 pt-12 sm:p-6 sm:pt-12">
          {active && <><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-bold">{active.name}</h2><Link onClick={onClose} href={'/products?category=' + encodeURIComponent(active.name)} className="text-xs underline">Browse all products</Link></div>
            {children.length === 0 && productsLoading && <p className="text-sm text-gray-500">Loading products…</p>}
            {children.length === 0 && productsError && <p className="text-sm text-gray-500">Products could not be loaded.</p>}
            <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 lg:grid-cols-5">
              {tiles.map((tile) => <Link key={tile.id} href={tile.href} onClick={onClose} className="group flex flex-col items-center gap-3 text-center">
                <div className="grid h-20 w-20 place-items-center overflow-hidden rounded-full bg-gray-100 sm:h-24 sm:w-24 group-hover:ring-2 group-hover:ring-orange-400">
                  {tile.image ? <img src={tile.image} alt={tile.name} className="h-full w-full object-contain" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <span className="px-2 text-center text-[10px] font-medium text-gray-400">No image</span>}
                </div><span className="text-xs sm:text-sm">{tile.name}</span>
              </Link>)}
              <Link href={'/products?category=' + encodeURIComponent(active.name)} onClick={onClose} className="flex flex-col items-center gap-3 text-center"><div className="grid h-20 w-20 place-items-center rounded-full bg-gray-100 sm:h-24 sm:w-24"><LayoutGrid size={30} className="text-gray-500" /></div><span className="text-sm">View all</span></Link>
            </div>
          </>}
        </section>
      </div>
